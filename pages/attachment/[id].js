...
import attachmentPageInitialPropsTqJOResource from '../../resources/attachment-page-initial-props-tq_j-o'
import attachmentPageInitialPathsTqAyResource from '../../resources/attachment-page-initial-paths-tq_ay'
...
export async function getStaticProps(context) {
  try {
    const response = await attachmentPageInitialPropsTqJOResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await attachmentPageInitialPathsTqAyResource({})
    return {
