...
import attachmentPageInitialPropsTqBjResource from '../../resources/attachment-page-initial-props-tq_bj'
import attachmentPageInitialPathsTqNResource from '../../resources/attachment-page-initial-paths-tq__n'
...
export async function getStaticProps(context) {
  try {
    const response = await attachmentPageInitialPropsTqBjResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await attachmentPageInitialPathsTqNResource({})
    return {
