...
import attachmentPageInitialPropsTqFSResource from '../../resources/attachment-page-initial-props-tq_f-s'
import attachmentPageInitialPathsTqJuResource from '../../resources/attachment-page-initial-paths-tq_ju'
...
export async function getStaticProps(context) {
  try {
    const response = await attachmentPageInitialPropsTqFSResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await attachmentPageInitialPathsTqJuResource({})
    return {
