...
import tagsPageInitialPropsTqX0Resource from '../../resources/tags-page-initial-props-tq_x0'
import tagsPageInitialPathsTqJzResource from '../../resources/tags-page-initial-paths-tq_jz'
...
export async function getStaticProps(context) {
  try {
    const response = await tagsPageInitialPropsTqX0Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await tagsPageInitialPathsTqJzResource({})
    return {
