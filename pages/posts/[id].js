...
import postsPageInitialPropsTqUeResource from '../../resources/posts-page-initial-props-tq_ue'
import postsPageInitialPathsTq8fResource from '../../resources/posts-page-initial-paths-tq_8f'
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsTqUeResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsTq8fResource({})
    return {
