...
import postPageInitialPropsTqIjResource from '../../resources/post-page-initial-props-tq_ij'
import postPageInitialPathsTqEFResource from '../../resources/post-page-initial-paths-tq_e-f'
...
export async function getStaticProps(context) {
  try {
    const response = await postPageInitialPropsTqIjResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await postPageInitialPathsTqEFResource({})
    return {
