...
import postPageInitialPropsTqORResource from '../../resources/post-page-initial-props-tq_o-r'
import postPageInitialPathsTqQfResource from '../../resources/post-page-initial-paths-tq_qf'
...
export async function getStaticProps(context) {
  try {
    const response = await postPageInitialPropsTqORResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await postPageInitialPathsTqQfResource({})
    return {
