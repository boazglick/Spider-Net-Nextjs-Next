...
import postPageInitialPropsTq7kResource from '../../resources/post-page-initial-props-tq_7k'
import postPageInitialPathsTqAuResource from '../../resources/post-page-initial-paths-tq_au'
...
export async function getStaticProps(context) {
  try {
    const response = await postPageInitialPropsTq7kResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await postPageInitialPathsTqAuResource({})
    return {
