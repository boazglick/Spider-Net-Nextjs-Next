...
import postPageInitialPropsTqFsResource from '../../resources/post-page-initial-props-tq_fs'
import postPageInitialPathsTqTsResource from '../../resources/post-page-initial-paths-tq_ts'
...
export async function getStaticProps(context) {
  try {
    const response = await postPageInitialPropsTqFsResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await postPageInitialPathsTqTsResource({})
    return {
