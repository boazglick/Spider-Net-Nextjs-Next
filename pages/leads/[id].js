...
import leadsPageInitialPropsTqJuResource from '../../resources/leads-page-initial-props-tq_ju'
import leadsPageInitialPathsTqShResource from '../../resources/leads-page-initial-paths-tq_sh'
...
export async function getStaticProps(context) {
  try {
    const response = await leadsPageInitialPropsTqJuResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await leadsPageInitialPathsTqShResource({})
    return {
